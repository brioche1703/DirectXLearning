...
Frustum::Frustum(Graphics& gfx, float width, float height, float nearZ, float farZ) {
...

	{
		...
		}
		{
			...
			occluded.AddBindable(PixelConstantBuffer<PSColorConstant>::Resolve(gfx, colorConst, 1u));
...
void Frustum::SetVertices(Graphics& gfx, float width, float height, float nearZ, float farZ) {
	...
	layout.Append(dxLearning::VertexLayout::Position3D);
	dxLearning::VertexBuffer vertices{ std::move(layout) };
