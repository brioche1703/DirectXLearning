...
#undef X
	...
	LayoutElement::LayoutElement(Type type_in) noxnd
		:
	type{ type_in }
	{
	...
	size_t LayoutElement::Finalize(size_t offset_in) noxnd {
		switch (type) {
#define X(el) case el: offset = AdvanceIfCrossesBoundary(offset_in, Map<el>::hlslSize); return *offset + Map<el>::hlslSize;
			LEAF_ELEMENT_TYPES
#undef X
		case Struct:
	...
	size_t LayoutElement::FinalizeForStruct(size_t offset_in) {
		...
		offset = AdvanceToBoundary(offset_in);
