...
#define DXLG_ELEMENT_AI_EXTRACTOR(member) static SysType Exctract(const aiMesh& mesh, size_t i) noexcept { return *reinterpret_cast<const SysType*>(&mesh.member[i]); }
...
namespace dxLearning {
	...
	public:
		...
		template<> struct Map<Position2D> {
			...
			DXLG_ELEMENT_AI_EXTRACTOR(mVertices)
		};
		...
		template<> struct Map<Texture2D> {
			...
			DXLG_ELEMENT_AI_EXTRACTOR(mTextureCoords[0])
		};
		template<> struct Map<Normal> {
			...
			DXLG_ELEMENT_AI_EXTRACTOR(mNormals)
		};
		template<> struct Map<Tangent>
		{
			...
			DXLG_ELEMENT_AI_EXTRACTOR(mTangents)
		};
		template<> struct Map<Bitangent>
		{
			...
			DXLG_ELEMENT_AI_EXTRACTOR(mBitangents)
		};
		template<> struct Map<Float3Color> {
			...
			DXLG_ELEMENT_AI_EXTRACTOR(mColors[0])
		};
		...
		template<> struct Map<Count> {
			...
			DXLG_ELEMENT_AI_EXTRACTOR(mFaces);
		};
	...
	private:
		// Recursive to set multiple parameters by element index
...
#undef DXLG_ELEMENT_AI_EXTRACTOR
#ifndef DXLG_SOURCE_FILE
